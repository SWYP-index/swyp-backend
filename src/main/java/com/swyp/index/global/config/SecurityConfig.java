...
import com.swyp.index.global.config.auth.OAuth2AuthenticationSuccessHandler;
import com.swyp.index.infrastructure.oauth.CustomOAuth2UserService;
...
public class SecurityConfig {
    ...
    //애플리케이션의 모든 HTTP 요청에 대한 보안 규칙을 정의하는 필터 체인을 설정
    @Bean
    public SecurityFilterChain securityFilterChain(HttpSecurity http, OAuth2AuthenticationSuccessHandler oAuth2AuthenticationSuccessHandler) throws Exception{
        http
                ...
                .authorizeHttpRequests(auth->auth
                        .requestMatchers("/api/auth/**", "/oauth2/**").permitAll() // '/api/auth/'로 시작하는 모든 경로는 인증 없이 허용
