...
public class BookDto {
	...
	public static BookDto from(Book book, List<BookStats> bookStats) {
		...
		List<BookStatsDto> statsDtos = mapBookStatsToDtos(book.getTotalEmotionScoreSum(), bookStats);
		...
		return BookDto.builder()
			...
			.bookStatsDtos(statsDtos)
			.build();
	}

	private static List<BookStatsDto> mapBookStatsToDtos(Long totalSum, List<BookStats> bookStats) {
		return bookStats.stream().map(bs -> {
			...
			return new BookStatsDto(emotionName, bs.getEmotionScoreSum(), percentage);
		}).toList();
	}
